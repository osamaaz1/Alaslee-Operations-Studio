...
إذا وجد السكربت تثبيت PostgreSQL قديماً بكلمة مرور مختلفة، فإنه لا يعدله ولا يحذف بياناته؛ ينشئ خدمة معزولة خاصة بـAlaslee على منفذ محلي حر ويحدّث `.env` تلقائياً.
...
## وضع الإنتاج داخل شبكة المحل

خادم التطوير على 5173 ليس خادم الإنتاج. بعد إنهاء الإعداد والقيم داخل `.env` شغّل:

```powershell
npm run production:test
```

ثم افتح PowerShell كمسؤول وشغّل مرة واحدة:

```powershell
.\scripts\install-production-windows.ps1 -EnvironmentFile .env
```

يعمل النظام بعدها تلقائياً على المنفذ 3000، وتعرض هذه الأداة رابط الشبكة الحالي:

```powershell
.\scripts\show-lan-url.ps1
```

- يجب أن يكون تصنيف شبكة المتجر في Windows هو **Private**.
- قاعدة PostgreSQL تبقى على loopback ولا تُفتح لبقية الأجهزة.
- قاعدة جدار الحماية تسمح لتطبيق Alaslee من `LocalSubnet` فقط.
- لأن العنوان مختار كـIPv4 متغير، قد يتغير الرابط بعد إعادة تشغيل الراوتر.
- لأن الاتصال المختار HTTP، يمنع توجيه المنفذ 3000 من الراوتر إلى الإنترنت.
- النسخ الاحتياطي لا يحتوي `.env` أو مفتاح تشفير CRM؛ احتفظ بنسخة استعادة مشفرة منفصلة.
