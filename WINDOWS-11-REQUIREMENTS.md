# متطلبات وتشغيل Alaslee Operations Studio على Windows 11

هذا الدليل يفترض جهاز Windows 11 جديداً لا يحتوي على أدوات تطوير. سكربت الإعداد يثبت الأدوات ويجهز المشروع تلقائياً، ولا يستبدل ملف `.env` موجوداً إلا عند طلب ذلك صراحة.

## متطلبات الجهاز

- Windows 11 بنظام 64-bit ومحدّث عبر Windows Update.
- صلاحية Administrator أثناء الإعداد الأول فقط.
- اتصال إنترنت لتنزيل Node.js وGit وPostgreSQL وحزم npm. Docker مطلوب فقط عند اختيار مساره البديل.
- مساحة خالية موصى بها: 15 GB على الأقل.
- ذاكرة: 8 GB كحد عملي أدنى، و16 GB موصى بها.
- لا يحتاج مسار التشغيل بدون Docker إلى Virtualization. يلزم تفعيلها فقط عند اختيار Docker/WSL 2.
- وجود ملفات المشروع في مسار محلي، والمسار الحالي المدعوم هو `D:\Codex\Alaslee-Operations-Studio`.

## ما الذي يتم تثبيته؟

عند تشغيل `setup-windows-no-docker.cmd` يثبت السكربت تلقائياً عبر WinGet:

1. Node.js LTS مع npm.
2. Git for Windows.
3. PostgreSQL 16 كخدمة Windows أصلية على المنفذ `5433`.
4. حزم المشروع بالإصدارات المقفلة في `package-lock.json` باستخدام `npm ci`.
5. قاعدة CRM وترحيلاتها.

هذا المسار لا يثبت Docker أو WSL ولا يحتاج Virtualization. يبقى `setup-windows.cmd` متاحاً كمسار بديل لمن يفضل Docker.
إذا وجد السكربت تثبيت PostgreSQL قديماً بكلمة مرور مختلفة، فإنه لا يعدله ولا يحذف بياناته؛ ينشئ خدمة معزولة خاصة بـAlaslee على منفذ محلي حر ويحدّث `.env` تلقائياً.

بعدها ينشئ `.env` للمرة الأولى بقيم محلية عشوائية، ويستخدم `free-test` افتراضياً كي يعمل النظام بلا حسابات AI مدفوعة، ثم يبني الواجهة ويشغل الاختبارات.

## التثبيت الأول

1. افتح مجلد المشروع.
2. انقر نقراً مزدوجاً على `setup-windows-no-docker.cmd` للتثبيت الأصلي الموصى به.
3. وافق على نافذة Administrator.
4. وافق على تثبيت PostgreSQL عند ظهوره وانتظر اكتمال إنشاء خدمة Windows وقاعدة CRM.

يسجل السكربت رقمي PIN عشوائيين للموظف والمشرف في `.env` ويعرضهما عند الإنشاء. لا ترفع `.env` إلى Git ولا تشاركه.

## التشغيل اليومي

للتطوير بدون Docker، نفّذ `setup-windows-no-docker.cmd` مرة واحدة ثم انقر نقراً مزدوجاً على `start-local.cmd`. يشغّل هذا المسار النظام كاملاً مع CRM عبر PostgreSQL الأصلي.

بعد تثبيت وضع الإنتاج في القسم التالي لا يلزم تشغيل أي ملف يومياً: تبدأ خدمة `AlasleeOperationsStudio` تلقائياً في الخلفية مع Windows بلا نافذة CMD أو PowerShell.

يبقى `setup-windows.cmd` ثم `start-local-with-docker.cmd` مساراً بديلاً اختيارياً فقط للأجهزة التي تدعم Docker.

افتح:

- الواجهة: `http://localhost:5173`
- API: `http://localhost:3000`

أوقف النظام بالضغط على `Ctrl+C` في نافذة التشغيل. عند استخدام نسخة Docker، تبقى بيانات PostgreSQL محفوظة في Docker volume ولا تضيع عند إيقاف الحاوية.

## وضع الإنتاج داخل شبكة المحل

خادم التطوير على 5173 ليس خادم الإنتاج. بعد إنهاء الإعداد والقيم داخل `.env` شغّل:

```powershell
npm run production:test
```

ثم افتح PowerShell كمسؤول وشغّل مرة واحدة:

```powershell
.\scripts\install-production-windows.ps1 -EnvironmentFile .env
```

يعمل النظام بعدها تلقائياً على المنفذ 3000 كخدمة Windows خفيفة وصامتة، وتعرض هذه الأداة رابط الشبكة الحالي:

```powershell
.\scripts\show-lan-url.ps1
```

- يجب أن يكون تصنيف شبكة المتجر في Windows هو **Private**.
- قاعدة PostgreSQL تبقى على loopback ولا تُفتح لبقية الأجهزة.
- قاعدة جدار الحماية تسمح لتطبيق Alaslee من `LocalSubnet` فقط.
- يسجل Windows الخدمة للتشغيل التلقائي وإعادة التشغيل بعد الأعطال؛ لا يعاد بناء الواجهة ولا يشغّل npm في كل إقلاع.
- عند إيقاف الخدمة أو إعادة تشغيل/إغلاق Windows، تطلب الخدمة إغلاقاً تدريجياً قبل إنهاء Node.js حتى تُغلق طلبات HTTP وقواعد البيانات بصورة سليمة.
- السجلات في `diagnostics\logs\server-service-YYYYMMDD.log` وملف الأخطاء المقابل، وتُحذف سجلات الخدمة الأقدم من 14 يوماً تلقائياً.
- لأن العنوان مختار كـIPv4 متغير، قد يتغير الرابط بعد إعادة تشغيل الراوتر.
- لأن الاتصال المختار HTTP، يمنع توجيه المنفذ 3000 من الراوتر إلى الإنترنت.
- النسخ الاحتياطي لا يحتوي `.env` أو مفتاح تشفير CRM؛ احتفظ بنسخة استعادة مشفرة منفصلة.

## مفاتيح الذكاء الاصطناعي (اختياري)

الوضع المحلي المجاني لا يستدعي خدمات AI. لاستخدام التوليد الفعلي، عدّل `.env` واختر أحد الخيارين:

```ini
AI_PROVIDER=gemini
GEMINI_API_KEY=ضع_المفتاح_هنا
```

أو:

```ini
AI_PROVIDER=gpt
OPENAI_API_KEY=ضع_المفتاح_هنا
```

ثم أوقف النظام وشغّل `start-local.cmd` من جديد.

## خيارات متقدمة وحل المشكلات

شغّل الخيارات التالية من PowerShell داخل مجلد المشروع:

```powershell
# إعادة إنشاء .env (يحفظ نسخة احتياطية أولاً)
.\setup-windows.ps1 -ForceEnvironment

# تثبيت PostgreSQL الأصلي وتجهيز CRM بدون Docker
.\setup-windows.ps1 -SkipDocker

# تخطي البناء والاختبارات لتجهيز أسرع
.\setup-windows.ps1 -SkipTests
```

- إذا لم يتوفر `winget`، ثبّت أو حدّث **App Installer** من Microsoft Store ثم أعد المحاولة.
- إذا أبلغ Docker أن virtualization غير متاح، فعّله من UEFI/BIOS ثم أعد تشغيل Windows.
- إذا بقي Docker في وضع Starting، نفّذ `wsl --update` من PowerShell كمسؤول ثم أعد تشغيل الجهاز.
- إذا كان المنفذ `5433` مستخدماً، غيّر `CRM_POSTGRES_PORT` وموضع المنفذ داخل `CRM_DATABASE_URL` في `.env` إلى الرقم نفسه.
- لا تشغّل `npm install` يدوياً للتجهيز النظيف؛ استخدم السكربت أو `npm ci` للمحافظة على الإصدارات المقفلة.
