...
import { ContactRound, LoaderCircle, PackageSearch, Settings2, ShoppingBag } from "lucide-react";
...
const views = [
  ...
  ["sales", "المبيعات اليدوية", ShoppingBag],
...
export function CrmWorkspace({ inform }) {
  ...
  return <section className="section-stack"><header className="page-title"><div><p className="eyebrow">إدارة العملاء</p><h1>علاقة أذكى مع كل عميل.</h1><p>ملف موحّد، كشف بصري محمي، مشتريات يدوية، وتصنيف واضح مبني على القيمة.</p></div><span className={`crm-role ${session.role}`}>{session.role === "superuser" ? "المشرف الأعلى" : "الموظفون"}</span></header>
    ...
    {view === "sales" && <SaleWorkspace session={session} inform={inform} />}
