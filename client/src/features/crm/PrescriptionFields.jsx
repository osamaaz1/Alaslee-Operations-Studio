...
import { RX_LIMITS } from "../../../../shared/crm/constants.js";

export const emptyPrescription = {
  consent: false, examDate: new Date().toISOString().slice(0, 10),
...
export function PrescriptionFields({ value, onChange, errors = {} }) {
  ...
  return <fieldset className="rx-card">
    ...
    <div className="pd-row"><Field label="طريقة قياس IPD"><select value={value.pdMode} onChange={(e) => set("pdMode", e.target.value)}><option value="binocular">قياس واحد IPD</option><option value="monocular">يمين ويسار PD</option></select></Field>
      {value.pdMode === "binocular" ? <RxControl label="IPD" value={value.binocularPd} range={RX_LIMITS.binocularPd} exceptional={value.exceptionalEntry} onChange={(next) => set("binocularPd", next)} error={errors.binocularPd} /> : <><RxControl label="PD يمين" value={value.rightPd} range={RX_LIMITS.monocularPd} exceptional={value.exceptionalEntry} onChange={(next) => set("rightPd", next)} error={errors.rightPd} /><RxControl label="PD يسار" value={value.leftPd} range={RX_LIMITS.monocularPd} exceptional={value.exceptionalEntry} onChange={(next) => set("leftPd", next)} error={errors.leftPd} /></>}</div>
    ...
    <label className="consent-row"><input type="checkbox" checked={value.consent} onChange={(e) => set("consent", e.target.checked)} /><span>وافق العميل صراحةً على حفظ بيانات الكشف الطبي الحساسة.</span></label>
...
function EyeFields({ title, side, value, onChange, exceptional, errors }) {
  return <section><h3>{title}</h3><div className="rx-grid">
    <RxControl label="SPH" value={value.sph} range={RX_LIMITS.sph} exceptional={exceptional} onChange={(next) => onChange(`${side}.sph`, next)} error={errors[`${side}.sph`]} />
    <RxControl label="CYL" value={value.cyl} range={RX_LIMITS.cyl} exceptional={exceptional} onChange={(next) => onChange(`${side}.cyl`, next)} error={errors[`${side}.cyl`]} />
    <RxControl label="Axis" value={value.axis} range={RX_LIMITS.axis} exceptional={exceptional} onChange={(next) => onChange(`${side}.axis`, next)} error={errors[`${side}.axis`]} disabled={Number(value.cyl || 0) === 0} />
    <RxControl label="ADD" value={value.add} range={RX_LIMITS.add} exceptional={exceptional} onChange={(next) => onChange(`${side}.add`, next)} error={errors[`${side}.add`]} optional />
  </div></section>;
}

function RxControl({ label, value, range, exceptional, onChange, error, disabled, optional }) {
  const values = rangeValues(range);
  return <Field label={label} error={error}>{exceptional ? <input type="number" step={range[2]} value={value} disabled={disabled} onChange={(e) => onChange(e.target.value)} /> : <select value={value} disabled={disabled} onChange={(e) => onChange(e.target.value)}>{optional && <option value="">—</option>}{values.map((item) => <option value={item} key={item}>{signed(item)}</option>)}</select>}</Field>;
}
...
function signed(value) { const number = Number(value); return number > 0 ? `+${number.toFixed(2)}` : number.toFixed(2); }
