...
function InventoryDetail({ product }) {
  ...
  return <article className="panel inventory-detail"><header className="crm-panel-title"><div><p className="eyebrow">تفاصيل المنتج</p><h2>{product.name}</h2></div><span className={`stock-pill ${stockState(product).tone}`} dir="ltr">{stockState(product).label}</span></header><div className="inventory-detail-grid"><section><h3>معرّفات المنتج</h3><Identifiers product={product} /><dl><Detail label="العلامة" value={product.brand || "غير مسجلة"} /><Detail label="التصنيف" value={product.category || "غير مسجل"} /><Detail label="حالة المنتج" value={product.status || "غير معروفة"} /><Detail label="آخر تحديث" value={product.synced_at ? new Date(product.synced_at).toLocaleString("ar-SA-u-nu-latn") : "غير متاح"} /></dl></section><section><h3>توزيع المخزون حسب الموقع</h3>{locations.length ? <div className="warehouse-grid">{locations.map((location) => <article key={location.storeId} className="warehouse-card"><MapPin size={17} /><span>{location.storeName || "موقع غير مسمى"}</span><strong dir="ltr">{quantity.format(Number(location.quantity || 0))}</strong></article>)}</div> : <p className="empty-copy">لا توجد مواقع مخزون مسجلة لهذا المنتج.</p>}</section><section><h3>ضوابط السعر</h3><div className="inventory-prices"><Detail label="سعر البيع" value={money.format(Number(product.unit_price || 0))} ltr /><Detail label="أقل سعر" value={product.minimum_price != null ? money.format(Number(product.minimum_price)) : "غير مسجل"} ltr /><Detail label="تتبع المخزون" value={product.track_stock === false ? "غير مفعّل" : "مفعّل"} /></div></section></div></article>;
}
