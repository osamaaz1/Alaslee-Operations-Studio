...
export function InventoryWorkspace({ inform }) {
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      available: tracked.filter((product) => availableQuantity(product) > 0).length,
      low: tracked.filter((product) => { const value = availableQuantity(product); return value > 0 && value <= 3; }).length,
      unavailable: tracked.filter((product) => availableQuantity(product) <= 0).length,
    };
  }, [products]);
...
function InventoryDetail({ product }) {
  ...
  return <article className="panel inventory-detail"><header className="crm-panel-title"><div><p className="eyebrow">تفاصيل المنتج</p><h2>{product.name}</h2></div><span className={`stock-pill ${stockState(product).tone}`} dir="ltr">{stockState(product).label}</span></header><div className="inventory-detail-grid"><section><h3>معرّفات المنتج</h3><Identifiers product={product} /><dl><Detail label="العلامة" value={product.brand || "غير مسجلة"} /><Detail label="التصنيف" value={product.category || "غير مسجل"} /><Detail label="حالة المنتج" value={product.status || "غير معروفة"} /><Detail label="مخزون دفترة" value={quantity.format(Number(product.stock_balance || 0))} ltr /><Detail label="محجوز محلياً" value={quantity.format(Number(product.reserved_quantity || 0))} ltr /><Detail label="المتاح للبيع" value={quantity.format(availableQuantity(product))} ltr /><Detail label="آخر تحديث" value={product.synced_at ? new Date(product.synced_at).toLocaleString("ar-SA-u-nu-latn") : "غير متاح"} /><Detail label="تتبع المخزون" value={product.track_stock === false ? "غير مفعّل" : "مفعّل"} /></dl></section><section><h3>توزيع المخزون حسب الموقع</h3>{locations.length ? <div className="warehouse-grid">{locations.map((location) => <article key={location.storeId} className="warehouse-card"><MapPin size={17} /><span>{location.storeName || "موقع غير مسمى"}</span><strong dir="ltr">{quantity.format(Number(location.quantity || 0))}</strong></article>)}</div> : <p className="empty-copy">لا توجد مواقع مخزون مسجلة لهذا المنتج.</p>}</section></div></article>;
}
...
function stockState(product) { if (product.track_stock === false) return { tone: "untracked", label: "غير متتبع" }; const value = availableQuantity(product); if (value <= 0) return { tone: "out", label: "0 · غير متوفر" }; if (value <= 3) return { tone: "low", label: `${quantity.format(value)} · منخفض` }; return { tone: "available", label: `${quantity.format(value)} · متوفر` }; }
function isOutOfStock(product) { return product.track_stock !== false && availableQuantity(product) <= 0; }
function availableQuantity(product) { return Number(product.available_quantity ?? product.stock_balance ?? 0); }
